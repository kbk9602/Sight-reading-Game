...
function getMIDIMessage(message) {
    ...
    if (command === 144 && velocity > 0) {
        const now = Date.now();
        // 1초 이상 입력이 없었을 경우 이전의 누적된 찌꺼기(stuck notes)를 청소
        if (now - lastNoteOnTime > 1000) {
            activeNotes.clear();
        }
        lastNoteOnTime = now;
        ...
        // 여러 음이 동시에 눌릴 때까지 아주 잠깐(80ms) 기다린 후 체크
        if (timer) clearTimeout(timer);
        timer = setTimeout(sendToGame, 80);
...
function sendToGame() {
    if (activeNotes.size > 0) {
        ...
        if (typeof pressMidi === 'function') {
            pressMidi(notes);
        } else if (typeof press === 'function') {
